...
class Settings(BaseSettings):
    ...
    def _validate_required(self) -> None:
        ...
        if is_test or self.APP_ENV in {"development", "test"}:
            # In non-production environments generate an ephemeral SECRET_KEY so
            # the app (and JWT helpers in core.security) still run when one is
            # not configured. This value is never used in production.
        ...
        # PAYSTACK_SECRET_KEY is intentionally NOT required: payment keys are managed
        # in the Admin Settings page (DB), with the env var only an optional fallback.
