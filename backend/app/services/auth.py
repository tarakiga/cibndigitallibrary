...
def authenticate_cibn_member(db: Session, login_data: CIBNMemberLogin) -> tuple[User, str]:
    ...
    if not user:
        ...
        # Build a valid email — the MSSQL view may not have an Email column
        member_email = external_member_data.get("Email") or external_member_data.get("EmailAddress") or ""
        if not member_email or not member_email.strip() or "@" not in member_email:
            member_email = f"{login_data.cibn_employee_id}@cibn.org"
        ...
        # Fix existing users that may have an invalid email from earlier bugs
        if not user.email or "@" not in user.email:
            user.email = f"{login_data.cibn_employee_id}@cibn.org"
            db.commit()
            db.refresh(user)
