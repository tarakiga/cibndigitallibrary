...
export function ContentCard({ 
  ...
  const handleAddToLibrary = async (e: React.MouseEvent) => {
    ...
    if (!user) {
      toast.info('Sign in to add items to your library')
    ...
    try {
      setIsLoading(true)
      const { contentService } = await import('@/lib/api/content')
      const response = await contentService.addToLibrary(item.id)
      toast.success(response.message || 'Added to your library')
      
      // Update local storage to reflect purchase immediately
      const purchasedStr = localStorage.getItem('purchased_content')
      const purchased = purchasedStr ? JSON.parse(purchasedStr) : []
      if (!purchased.some((p: any) => p.id === item.id)) {
        purchased.push(item)
        localStorage.setItem('purchased_content', JSON.stringify(purchased))
        window.dispatchEvent(new Event('library:updated'))
      }
      
      // Redirect to library since they just got it
      router.push('/my-library')
    } catch (error: any) {
      console.error('Failed to add to library:', error)
      toast.error(error.response?.data?.detail || 'Failed to add to library')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const isActiveCibnMember = user?.role === 'cibn_member' && 
    Number(user?.arrears || 0) === 0 && 
    Number(user?.annual_subscription || 0) === 0;
    
  const isFreeItem = !item.price || item.price === 0 || item.is_exclusive || isActiveCibnMember;
        ...
        <div className={`flex-1 ${viewMode === 'list' ? 'p-6' : ''}`}>
          ...
          <CardFooter className={viewMode === 'list' ? 'pt-0' : 'pt-0'}>
            <div className="flex items-center justify-between w-full">
              <div>
                {isFreeItem ? (
                  <p className="text-2xl font-bold text-[#059669]">Free</p>
                ) : (
              ...
              {showActions && (
                <div className="flex items-center gap-2">
                  {user?.role === 'admin' ? (
                    ...
                    // Free items & exclusive items: "Add to Library" button
                    <Button 
                      size="sm" 
                    ...
                    // Paid items: regular "Add to Cart" button
                    <Button 
                      size="sm" 
                      className="gap-2" 
